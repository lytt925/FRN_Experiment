...
def choice(win, thisExp, routineTimer, globalClock, defaultKeyboard, serialPort):

    # --- Initialize components for Routine "choice" ---
    img_choice = visual.ImageStim(
        win=win,
        name='img_choice', 
    ...
    [img_choice.image] = randchoice(['Stimuli/Int_1.png', 'Stimuli/Int_2.png'], size=1)
    if img_choice.image == 'Stimuli/Int_1.png':
    ...
    while continueRoutine and routineTimer.getTime() < 3.0:
        ...
        if key_choice.status == STARTED and not waitOnFlip:
            ...
            if len(_key_choice_allKeys):
                ...
                thisExp.addData('select_key_trigger', globalClock.getTime())
                serialPort.write(bytes(trigger_map[_key_choice_allKeys[-1].name], 'utf8'))
        ...
        if continueRoutine:  # don't flip if this routine is over or we'll get a blank screen
            ...
            if not sendTrigger:
                # send trigger to port
                serialPort.write(bytes(chr(94), 'utf8'))
                thisExp.addData('jar_select_trigger', globalClock.getTime())
                sendTrigger = True
    ...
    return jar_img_map, trial_choice